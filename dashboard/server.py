"""
dashboard/server.py — TITAN Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "TITAN Uploads",
        Path.home() / "Documents" / "TITAN Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'TITAN-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"TITAN Dashboard Port {port}"
        prog_rule  = "TITAN Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="titan_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until TITAN is run as Administrator.")
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        ...
        return (certs / "titan.key").exists() and (certs / "titan.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in TITAN to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('titan_token','{tok}');
  sessionStorage.setItem('titan_key','{key}');
  localStorage.setItem('titan_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to TITAN…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "titan.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "titan.crt"
