...
# ── Debug toggle: set True to see full audio pipeline timestamps ──
_DEBUG_AUDIO = True
_dbg_chunk_count = 0
...
class TitanLive:
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        return types.LiveConnectConfig(
            ...
            # Native-audio Live models silently run an internal reasoning pass
            # (visible in your logs as the 'thought' part) before producing
            # audio. That's most of your multi-second delay. Force it off.
            thinking_config=types.ThinkingConfig(
                thinking_budget=0,
                include_thoughts=False,
            ),
            # ── Latency fix #2: end-of-turn / VAD detection ───────────────────
            # Defaults wait quite a while after you stop talking before they
            # decide you're done and start responding. Tighten it.
            ...
            if not titan_speaking and not self.ui.muted and not self._phone_active:
                ...
                if _DEBUG_AUDIO:
                    try:
                        ...
                        if rms > 300.0:
                            _dbg("MIC 🎤", f"Mic captured speech (RMS={rms:.0f})")
                    except Exception:
                        pass
    ...
    async def _run_system_monitor(self) -> None:
        ...
        while True:
            ...
            if speaking or (time.monotonic() - getattr(self, "_last_user_speech", 0.0)) < 30:
                continue
            try:
                await self.session.send_client_content(
