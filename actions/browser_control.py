...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".titan_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".titan_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using TITAN profile")
                titan = str(Path.home() / ".titan_profiles" / "firefox_titan")
                Path(titan).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(titan, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".titan_profiles" / "safari")
        ...
        # TITAN otomasyon profiline geçilir — buraya bir kez giriş yapılan
        # hesaplar sonraki oturumlarda da açık kalır.
        titan_profile = str(Path.home() / ".titan_profiles" / self.browser_name)
        Path(titan_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with TITAN profile: {titan_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(titan_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with TITAN profile "
    ...
    async def _get_page(self) -> Page:
        ...
        try:
            ...
            if "closed" in str(e).lower():
                print("[Browser] Context closed — auto-resetting browser session...")
                self._context = None
                self._page = None
                await self._launch()
                return self._page if self._page and not self._page.is_closed() else await self._adopt_page()
            raise
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "titan_screenshot.png")
...
def browser_control(
    ...
    # Auto-delegate Chrome actions to shadow_link (uses real open Chrome window)
    if not browser or browser in ("chrome", "google chrome", "google-chrome"):
        try:
            from actions.shadow_link import shadow_link_control
            s_action = action
            if action in ("go_to", "search"): s_action = "navigate"
            elif action in ("smart_click", "click"): s_action = "click"
            elif action in ("smart_type", "type"): s_action = "input"

            s_params = dict(params)
            s_params["action"] = s_action
            if "description" in params and "text" not in s_params and "selector" not in s_params:
                s_params["text"] = params["description"]
                s_params["selector"] = params["description"]

            if action == "search" and "query" in params:
                s_params["url"] = f"https://www.google.com/search?q={params['query'].replace(' ', '+')}"

            s_res = shadow_link_control(s_params)
            if "Shadow-Link" in s_res:
                if "No extension connected" not in s_res and "Could not connect" not in s_res:
                    _log(player, f"[ShadowLink Auto] {s_res}")
                    return s_res
                # If extension is not connected (Chrome is closed), open Chrome natively!
                if action in ("go_to", "search"):
                    nav_url = s_params.get("url") or f"https://www.google.com/search?q={params.get('query','').replace(' ', '+')}"
                    return _open_native(nav_url, "chrome")
        except Exception as e:
            print(f"[BrowserControl] ShadowLink error: {e}")
