...
def _ask_for_url(prompt_text: str = "YouTube video URL:") -> str | None:
    try:
        ...
        url = simpledialog.askstring("TITAN", prompt_text, parent=root)
...
def _get_transcript(video_id: str) -> str | None:
    ...
    try:
        ...
        if transcript_list is not None:
            lang_priority = ["en", "tr", "hi", "de", "fr", "es", "it", "pt", "ru", "ja", "ko", "ar", "zh"]
            t_obj = None
            try:
                t_obj = transcript_list.find_manually_created_transcript(lang_priority)
            except Exception:
                pass
            if t_obj is None:
                try:
                    t_obj = transcript_list.find_generated_transcript(lang_priority)
                except Exception:
            ...
            if t_obj:
                ...
                if isinstance(fetched, list):
                    return " ".join(entry.get("text", "") if isinstance(entry, dict) else str(entry) for entry in fetched)
                elif hasattr(fetched, "snippets"):
                    return " ".join(getattr(s, "text", str(s)) for s in fetched.snippets)
                elif hasattr(fetched, "text"):
                    return fetched.text
        ...
        if hasattr(api, "fetch"):
            ...
            if isinstance(fetched, list):
                ...
                return " ".join(getattr(s, "text", str(s)) for s in fetched.snippets)
            elif hasattr(fetched, "raw"):
                return " ".join(entry.get("text", "") for entry in fetched.raw)
