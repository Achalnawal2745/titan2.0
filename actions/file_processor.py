"""
file_processor.py — TITAN Universal File Processor
...
def _process_pdf(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    if action in ("summarize", "extract_text", "translate_hint", "analyze", "reformat"):
        ...
        if action == "extract_text" and text.strip():
        ...
        if not text.strip():
            try:
                ...
                prompt_map = {
                    "summarize":      "Summarize this PDF document concisely and thoroughly.",
                    "analyze":        "Analyze this document in detail.",
                    "extract_text":   "Extract and transcribe all text from this PDF document.",
                    "translate_hint": "What language is this document in and what does it say?",
                    "reformat":       "Reformat and structure the content of this PDF clearly.",
                }
                response = c.models.generate_content(
                    model="gemini-2.5-flash",
                    contents=[part, prompt_map.get(action, "Summarize this PDF document.")]
                )
                result = response.text.strip()
                if action == "extract_text":
                    out = _output_path(path, "text", ".txt")
                    out.write_text(result, encoding="utf-8")
                    return f"Text extracted via Gemini Multimodal ({len(result)} chars). Saved: {out.name}"
                ...
                return f"Gemini PDF processing failed: {e}"
    ...
    def _read_content() -> str:
        if file_type == "docx":
            try:
                ...
                # Built-in zipfile fallback for .docx files (zero dependencies)
                try:
                    import zipfile
