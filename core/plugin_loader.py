...
Discovery runs once (TitanLive.__init__ calls discover_plugins()); the resulting
...
def discover_plugins(plugins_dir: Path, core_tool_names: set[str],
    ...
    for path in files:
        ...
        if rec.valid:
            ...
            if "collides with a core tool" not in rec.error:
                try:
                    logger(f"SYS: [!] Plugin rejected: {path.name} - {rec.error}")
                except Exception:
                    pass
    ...
    try:
        logger(f"[PLUGINS] {len(valid)} Live Voice Plugins Loaded & Active")
    except Exception:
        pass
    return registry
