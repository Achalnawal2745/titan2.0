...
class LogWidget(QTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        self._pos     = 0
        self._tag     = "sys"
        self._ai_name_lc = "titan"   # updated when assistant name changes
        self._tmr = QTimer(self)
        self._tmr.timeout.connect(self._step)
    ...
    def _next(self):
        ...
        elif tl.startswith(_ai_pfx) or tl.startswith("titan:"): self._tag = "ai"
...
class ClipboardPanel(QWidget):
    """Floating panel shown when text is copied — offers quick Titan actions."""
...
class RemoteKeyOverlay(QWidget):
    ...
    def mark_connected(self) -> None:
        ...
        self._timer_lbl.setText("Phone connected — TITAN ready")
    ...
    def _do_close(self):
        ...
        self.hide()
        self.closed.emit()
# ── Face Enrollment Dialog with Camera Preview & Capture Button ─────────────
# ── Face Enrollment Dialog with Camera Preview & Capture Button ─────────────
class FaceEnrollDialog(QDialog):
    """Visual face enrollment dialog displaying live webcam feed and a CAPTURE FACE button."""
    _frame_sig = pyqtSignal(QPixmap)

    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("👤 Face Enrollment — TITAN")
        self.setFixedSize(480, 420)
        self.setStyleSheet("background: #000d14; color: #d8f8ff;")
        self.captured_bytes = None
        self._running = True
        self._frame_sig.connect(self.feed_lbl_update)
        ...
        self.feed_lbl = QLabel("Initializing camera...")
        self.feed_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.feed_lbl.setStyleSheet("background: #00060a; border: 1px solid #007a99; border-radius: 6px;")
        self.feed_lbl.setFixedHeight(300)
        ...
        capture_btn.setStyleSheet("""
            QPushButton {
                background: #00aa55; color: #ffffff;
                border: 1px solid #00ff88; border-radius: 4px;
            }
            QPushButton:hover { background: #00ff88; color: #000000; }
        """)
    ...
    def feed_lbl_update(self, pixmap: QPixmap):
        self.feed_lbl.setPixmap(pixmap)
            ...
            try:
                ...
                while self._running and cap.isOpened():
                    ...
                    if ret and frame is not None:
                        ...
                        pixmap = QPixmap.fromImage(qimg).scaled(440, 290, Qt.AspectRatioMode.KeepAspectRatio)
                        
                        _, buf = cv2.imencode('.jpg', frame)
                        self._last_bytes = buf.tobytes()
                        self._frame_sig.emit(pixmap)
                    time.sleep(0.03)
                cap.release()
            except Exception as e:
                print(f"[FaceEnrollDialog] Camera error: {e}")
        threading.Thread(target=_loop, daemon=True).start()

    def _do_capture(self):
        if hasattr(self, '_last_bytes') and self._last_bytes:
            self.captured_bytes = self._last_bytes
            self._running = False
            self.accept()
...
# ── Interactive Startup & Security Lock Screen Dialog ───────────────────────
class LockScreenDialog(QDialog):
    """
    Sleek Startup & Authentication Lock Screen Dialog.
    Blocks access to TITAN until owner authenticates via Face, Voice, or Passcode.
    """
    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("🔒 TITAN SECURITY LOCK")
        ...
        if parent:
            ...
            self.move((screen.width() - 500) // 2, (screen.height() - 420) // 2)
        ...
        self.face_btn.setStyleSheet("""
            QPushButton {
                background: #001a28; color: #00d4ff;
                border: 1px solid #007a99; border-radius: 5px;
            }
            QPushButton:hover { background: #002b3d; border-color: #00d4ff; }
        """)
        self.face_btn.clicked.connect(self._try_face)
        lay.addWidget(self.face_btn)
        ...
        self.voice_btn.setStyleSheet("""
            QPushButton {
                background: #001a18; color: #00ff88;
                border: 1px solid #00aa55; border-radius: 5px;
            }
            QPushButton:hover { background: #002515; }
        """)
        self.voice_btn.clicked.connect(self._try_voice)
        ...
        self.pin_input.setStyleSheet("""
            QLineEdit {
                background: #00060a; color: #d8f8ff;
                border: 1px solid #0d3347; border-radius: 4px; padding: 0 8px;
            }
            QLineEdit:focus { border-color: #00d4ff; }
        """)
        self.pin_input.returnPressed.connect(self._try_pin)
        ...
        pin_btn.setStyleSheet("""
            QPushButton {
                background: #001f2e; color: #00d4ff;
                border: 1px solid #007a99; border-radius: 4px; padding: 0 12px;
            }
            QPushButton:hover { background: #00334d; border-color: #00d4ff; }
        """)
        pin_btn.clicked.connect(self._try_pin)
        ...
        self.cancel_btn.setStyleSheet("""
            QPushButton {
                background: transparent; color: #ff5566;
                border: 1px solid #772233; border-radius: 4px;
            }
            QPushButton:hover { background: #330011; border-color: #ff3355; }
        """)
    ...
    def _try_face(self):
        self.status_lbl.setText("📸 Scanning face via webcam…")
        self.status_lbl.setStyleSheet("color: #00d4ff;")
        QApplication.processEvents()
        from actions.voice_face_id import _verify_face_live
        if _verify_face_live():
            self.status_lbl.setText("✅ Face Verified! Unlocking TITAN…")
            ...
            self.status_lbl.setText("❌ Face not matched. Try Voice or Passcode.")
            self.status_lbl.setStyleSheet("color: #ff3355;")

    def _try_voice(self):
        self.status_lbl.setText("🎙 Speak clearly into mic for 3 seconds…")
        self.status_lbl.setStyleSheet("color: #00ff88;")
        QApplication.processEvents()
        try:
            import sounddevice as sd
            audio = sd.rec(int(3 * 16000), samplerate=16000, channels=1, dtype='int16', blocking=True)
            from actions.voice_face_id import _verify_voice_sample
            if _verify_voice_sample(audio.tobytes()):
                self.status_lbl.setText("✅ Voice Verified! Unlocking TITAN…")
                self.status_lbl.setStyleSheet("color: #00ff88;")
                QApplication.processEvents()
                time.sleep(0.5)
                self.authenticated = True
                ...
                self.status_lbl.setText("❌ Voice not matched. Try Face or Passcode.")
                self.status_lbl.setStyleSheet("color: #ff3355;")
        except Exception as e:
            self.status_lbl.setText(f"❌ Voice check error: {e}")
            self.status_lbl.setStyleSheet("color: #ff3355;")

    def _try_pin(self):
        pin = self.pin_input.text().strip()
        from actions.voice_face_id import verify_pin
        if pin and verify_pin(pin):
            self.status_lbl.setText("✅ Passcode Verified! Unlocking TITAN…")
            self.status_lbl.setStyleSheet("color: #00ff88;")
            QApplication.processEvents()
            time.sleep(0.5)
            self.authenticated = True
            ...
            self.status_lbl.setText("❌ Incorrect Passcode. Try again.")
            self.status_lbl.setStyleSheet("color: #ff3355;")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self._assistant_name: str = (_cfg.get("assistant_name") or "TITAN").strip()
        ...
        self.setWindowTitle(f"{_display} — TITAN 2.0")
        ...
        self.on_interrupt      = None   # callable: () -> None — stop TITAN mid-speech
        self._muted            = False
        self._current_file: str | None = None
    ...
    def _build_quick_drawer(self) -> QWidget:
        ...
        lay.addWidget(self._enroll_face_btn)
        ...
        lay.addWidget(self._enroll_voice_btn)

        # Set Passcode row
        pin_row = QHBoxLayout(); pin_row.setSpacing(4)
        ...
        pin_set_btn.setStyleSheet(f"""
            ...
            QPushButton:hover {{ background: {C.PRI_GHO}; border-color: {C.PRI}; }}
        """)
        pin_set_btn.clicked.connect(self._set_passcode)
        pin_row.addWidget(pin_set_btn)
        lay.addLayout(pin_row)
        ...
        self._refresh_security_buttons()
    ...
    def _build_footer(self) -> QWidget:
        ...
        lay.addWidget(_fl("[Esc] Interrupt  ·  [F4] Mute  ·  [F11] Fullscreen"))
        lay.addStretch()
        lay.addWidget(_fl("By Achal", C.PRI_DIM))
        return w
    ...
    def _check_autostart(self) -> bool:
        ...
        try:
            if _OS == "Windows":
                ...
                try:
                    winreg.QueryValueEx(key, "TITAN_AI")
                    return True
                ...
                return (Path.home() / "Library" / "LaunchAgents"
                        / "com.titan.assistant.plist").exists()
            else:
                return (Path.home() / ".config" / "autostart" / "titan.desktop").exists()
        except Exception:
            return False
    ...
    def _toggle_autostart(self):
        ...
        try:
            ...
            if _OS == "Windows":
                ...
                if currently_on:
                    winreg.DeleteValue(reg, "TITAN_AI")
                else:
                    pythonw = Path(sys.executable).parent / "pythonw.exe"
                    exe = str(pythonw if pythonw.exists() else sys.executable)
                    winreg.SetValueEx(reg, "TITAN_AI", 0, winreg.REG_SZ,
                                      f'"{exe}" "{script}"')
                winreg.CloseKey(reg)
            elif _OS == "Darwin":
                plist_dir = Path.home() / "Library" / "LaunchAgents"
                plist_dir.mkdir(parents=True, exist_ok=True)
                plist = plist_dir / "com.titan.assistant.plist"
                if currently_on:
                    ...
                    plist.write_text(
                        ...
                        '"http://www.apple.com/DTDs/PropertyList-1.0.dtd">\n'
                        '<plist version="1.0"><dict>\n'
                        '  <key>Label</key><string>com.titan.assistant</string>\n'
    ...
    def _refresh_security_buttons(self):
        """Refresh security button labels/styles based on current config."""
        try:
            from actions.voice_face_id import get_security_config
        ...
        if hasattr(self, '_enroll_face_btn'):
            face_ok = cfg.get("face_enrolled")
            self._enroll_face_btn.setText("👤  FACE: ENROLLED ✓" if face_ok else "👤  ENROLL FACE")
            self._enroll_face_btn.setStyleSheet(_ON if face_ok else _OFF)

        if hasattr(self, '_enroll_voice_btn'):
            voice_ok = cfg.get("voice_enrolled")
            self._enroll_voice_btn.setText("🎙  VOICE: ENROLLED ✓" if voice_ok else "🎙  ENROLL VOICE (3s)")
            self._enroll_voice_btn.setStyleSheet(_ON if voice_ok else _OFF)

        if hasattr(self, '_master_sec_btn'):
            sec_on = cfg.get("security_lock", False)
            self._master_sec_btn.setText(f"{'🔒' if sec_on else '🔓'}  SECURITY LOCK: {'ON' if sec_on else 'OFF'}")
            self._master_sec_btn.setStyleSheet(_ON if sec_on else _OFF)

    def _authenticate_for_settings(self) -> bool:
        """Prompt for authentication before allowing security changes.
           Returns True if authenticated OR if no security is enrolled yet."""
        from actions.voice_face_id import get_security_config
        cfg = get_security_config()
        if not cfg["voice_enrolled"] and not cfg["face_enrolled"] and not cfg.get("pin_hash"):
            return True  # Nothing to authenticate against yet!
            
        dialog = LockScreenDialog(self)
        return dialog.exec() == QDialog.DialogCode.Accepted and dialog.authenticated

    def _enroll_face(self):
        """Open visual face enrollment dialog with camera preview and capture button."""
        if not self._authenticate_for_settings():
            self._log_sig.emit("[Security] ❌ Authentication required to enroll face.")
            return
            
        dialog = FaceEnrollDialog(self._win if hasattr(self, '_win') else None)
        if dialog.exec() == QDialog.DialogCode.Accepted and dialog.captured_bytes:
            from actions.voice_face_id import enroll_owner
            result = enroll_owner(face_image=dialog.captured_bytes)
            self._log_sig.emit(f"[Security] {result}")
            self._sec_refresh_sig.emit()

    def _enroll_voice(self):
        """Record 3 seconds of microphone audio and save owner voice embedding."""
        if not self._authenticate_for_settings():
            self._log_sig.emit("[Security] ❌ Authentication required to enroll voice.")
            return
            
        self._log_sig.emit("[Security] 🎙 Recording voice — speak for 3 seconds...")
        def _do():
            try:
                import sounddevice as sd
                import numpy as np
                ...
                self._log_sig.emit(f"[Security] ❌ Voice enrollment error: {e}")
        threading.Thread(target=_do, daemon=True).start()

    def _set_passcode(self):
        """Set owner passcode from the PIN input field."""
        if not self._authenticate_for_settings():
            self._log_sig.emit("[Security] ❌ Authentication required to change passcode.")
            return
            
        if not hasattr(self, '_pin_input'):
            return
        pin = self._pin_input.text().strip()
        if len(pin) < 4:
            self._log_sig.emit("[Security] ❌ Passcode must be at least 4 characters.")
            return
        from actions.voice_face_id import set_pin, set_master_security_lock
        set_pin(pin)
        set_master_security_lock(True)
        self._pin_input.clear()
        self._log_sig.emit("[Security] 🔒 Passcode set! Master Security Lock enabled.")
        self._sec_refresh_sig.emit()

    def _open_lock_screen_dialog(self):
        """Main GUI thread handler: display interactive LockScreenDialog modally."""
        dialog = LockScreenDialog(self)
        self._lock_authenticated = (dialog.exec() == QDialog.DialogCode.Accepted and dialog.authenticated)
        self._lock_auth_event.set()

    def _toggle_master_security(self):
        from actions.voice_face_id import get_security_config, set_master_security_lock, _verify_face_live
        cfg = get_security_config()
        current_state = cfg.get("security_lock", False)

        # TURNING ON — allowed freely if enrolled
        if not current_state:
            if not cfg.get("voice_enrolled") and not cfg.get("face_enrolled") and not cfg.get("pin_hash"):
                self._log_sig.emit("[Security] ❌ Enroll face/voice or set a passcode first.")
                return
            set_master_security_lock(True)
            self._log_sig.emit("[Security] Master Security Lock: ON 🔒")
            self._refresh_security_buttons()

        # TURNING OFF — REQUIRES AUTHENTICATION!
        else:
            self._log_sig.emit("[Security] 🔒 Authentication required to disable security lock.")
            if self._authenticate_for_settings():
                set_master_security_lock(False)
                self._log_sig.emit("[Security] ✅ Identity verified! Master Security Lock: OFF 🔓")
            else:
                self._log_sig.emit("[Security] ❌ Authentication failed! Lock remains ON.")
            self._refresh_security_buttons()
    ...
    def _on_setup_done(self, key: str, os_name: str):
        ...
        if self._overlay:
            ...
            self._overlay = None
        self._apply_state("LISTENING")
        self._assistant_name = _read_full_config().get("assistant_name", "TITAN") or "TITAN"
...
class TitanUI:
    ...
    def refresh_security_ui(self):
        ...
        self._win._sec_refresh_sig.emit()

    def show_face_enroll_dialog(self):
        """Thread-safe: trigger main thread to open visual face camera window."""
        self._win._enroll_face_sig.emit()

    def show_lock_screen_dialog(self) -> bool:
        """Thread-safe: trigger main thread to display LockScreenDialog and block until done."""
        self._win._lock_auth_event.clear()
        self._win._lock_authenticated = False
        self._win._lock_screen_sig.emit()
        self._win._lock_auth_event.wait()
        return self._win._lock_authenticated
